...
while cap.isOpened():
    ...
    if cur_time - prev_time > 1./20:  # 20 프레임으로 제한
        ...
        if result.multi_hand_landmarks is not None:
            ...
            if len(joint_list) == 2 or (len(joint_list) == 1 and actions[idx] in ['cough', 'runnynose', 'painful']):
                if len(joint_list) == 1:
                    joint_list.append(np.zeros((21, 4 * 5)))  # 인식되지 않은 손에 대한 0 패딩
                d = np.concatenate(joint_list)  # 두 손의 데이터를 결합
                d = np.concatenate([d, [0]])  # 마지막에 1차원 추가하여 199차원으로 맞춤
                seq.append(d)
